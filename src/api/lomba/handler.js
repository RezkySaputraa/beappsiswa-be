...
class CompetitionsHandler {
  ...
  postLombaHandler = async (request, h) => {
    try {
      ...
      if (payload.timeline) {
        if (payload.timeline.pendaftaran_mulai)
          payload.timeline.pendaftaran_mulai = new Date(
            payload.timeline.pendaftaran_mulai
          );
        if (payload.timeline.pendaftaran_selesai)
          payload.timeline.pendaftaran_selesai = new Date(
            payload.timeline.pendaftaran_selesai
          );
        if (payload.timeline.pengumpulan_karya)
          payload.timeline.pengumpulan_karya = new Date(
            payload.timeline.pengumpulan_karya
          );
        if (payload.timeline.deadline_karya)
          payload.timeline.deadline_karya = new Date(
            payload.timeline.deadline_karya
          );
        if (payload.timeline.pengumuman)
          payload.timeline.pengumuman = new Date(payload.timeline.pengumuman);
      }
